"""
CLI sub-command: ``threatflow plan``

    threatflow plan --attack-technique T1059.001
    threatflow plan --attack-technique T1486 --provider crowdstrike
"""
...
from threatflow.mappings.mitre import MitreIndex
...
def plan(
    ...
    defensive techniques, then lists ThreatFlow actions that implement them.

    Examples::

        threatflow plan --attack-technique T1486
        threatflow plan --attack-technique T1059.001 --provider crowdstrike
    """
    ...
    if not actions:
        console.print(
            f"\n[yellow]No ThreatFlow actions found for {technique_id}"
    ...
    console.print(table)
    console.print(
        f"\n[dim]Run [bold]threatflow run <action_id> --provider <provider>[/bold] to execute.[/dim]"
    )
