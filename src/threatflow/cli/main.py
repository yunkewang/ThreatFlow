"""
ThreatFlow CLI — main entry point.

Provides the ``threatflow`` command with sub-groups:
    threatflow actions list
    threatflow actions show <id>
    threatflow run <action_id> --provider <provider>
    threatflow plan --attack-technique <technique_id>
    threatflow playbook validate <file>
    threatflow playbook run <file> --inputs <json_file>
"""
...
app = typer.Typer(
    name="threatflow",
    help=(
        "ThreatFlow — vendor-neutral SOC response abstraction framework.\n\n"
...
def version_callback(value: bool) -> None:
    if value:
        console.print(f"threatflow v{__version__}")
        raise typer.Exit()
...
def global_options(
    ...
    """ThreatFlow — vendor-neutral SOC response framework."""
    if verbose:
...
def run_action(
    ...
    Examples:
        ...
        threatflow run isolate_host --provider crowdstrike --param host_id=abc123 --dry-run

        threatflow run block_ip --provider defender --param ip_address=1.2.3.4 --force
    """
