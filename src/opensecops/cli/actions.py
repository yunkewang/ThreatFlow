"""
CLI sub-commands: ``opensecops actions``

    opensecops actions list [--domain D] [--provider P] [--tag T]
    opensecops actions show <action_id>
"""
...
def actions_list(
    ...
    table = Table(
        title=f"OpenSecOps Action Catalog ({len(actions)} action(s))",
        box=box.ROUNDED,
        show_header=True,
    ...
    console.print(
        ...
        f"Run [bold]opensecops actions show <id>[/bold] for details.[/dim]"
    )
