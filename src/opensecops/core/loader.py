...
can be overridden via ``OPENSECOPS_CATALOG_DIR`` or by passing a directory to
...
class CatalogLoader:
    ...
    1. ``OPENSECOPS_CATALOG_DIR`` environment variable.
    ...
    def _resolve_catalog_dir(self) -> Path:
        ...
        env_override = os.environ.get("OPENSECOPS_CATALOG_DIR")
        if env_override:
